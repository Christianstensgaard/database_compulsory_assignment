...
public class AuthorModel{
    ...
    try{
      ...
      if (reader.Read()){
        var authorModel = new AuthorModel
        {
          ...
          BioId = reader["BioID"].ToString()
        };

        if (!string.IsNullOrEmpty(authorModel.BioId)){
          var bioCollection = MongoDb.I.GetCollection("Bios");

          var filter = Builders<BsonDocument>.Filter.Eq("_id", new ObjectId(authorModel.BioId));
          var bioDocument = bioCollection.Find(filter).FirstOrDefault();

          if (bioDocument != null)
          {
              authorModel.Bio = bioDocument["Bio"].ToString();
          }
        }
        return authorModel;
      }
  ...
  public static bool CreateAuthor(AuthorModel model){
    ...
    try
    {
      var bioId = MDb.InsertBio(model.Bio);
